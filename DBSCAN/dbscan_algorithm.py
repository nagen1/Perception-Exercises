...
def cluster_gen(n_clusters, pts_minmax=(10, 100), x_mult=(1, 4), y_mult=(1, 3),
    ...
    # x_mult = range of multipler to modify the size of cluster in the x-direction
    # y_mult = range of multipler to modify the size of cluster in the y-direction
    # x_off = range of cluster position in the x-direction
    # y_off = range of cluster positions in the y-direction
    ...
    for idx, npts in enumerate(n_points):
        ...
        cluster_x.append(xpts)
        cluster_y.append(ypts)
...
max_distance = 1
db = DBSCAN(eps=max_distance, min_spaces=10).fit(data)
...
plt.xlim(min_x, max_x)
plt.xlim(min_y, max_y)
plt.title('DBSACN: %d clusters found' %n_clusters, fontsize=20)
fig.tight_layout()
plt.tight_layout()
